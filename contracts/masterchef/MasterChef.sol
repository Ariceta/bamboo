...
contract MasterChef is Ownable {
    ...
    // The structure is amount => (staked time => multiplier)
    mapping (uint256 => mapping (uint256 => uint256)) public multipliers;
    // Info of each user that stakes BAMBOO.
    mapping (uint256 => mapping (address => BaUserInfo)) public baUserInfo;
    ...
    // Return reward multiplier over the given the time in seconds of the staking and the amount to lock
    function getStakerMultiplier(uint256 _time, uint256 _amount) public view returns (uint256) {
